 ...
 * @author gerson
 */
public class AdicionInscripcionController extends AbstractController {
    private DaoConexion daoConexion;
    
    public AdicionInscripcionController() {
    }
    ...
    protected ModelAndView handleRequestInternal(
        ...
        String mensaje = "";
        String pagina = "principal/adicionInscripcion";
                ...
                if ("POST".equals(request.getMethod())) {
                    ...
                    String action = request.getParameter("action").toString();
                     if (request.getParameter("action").equals("Guardar")) {
                        //Buscar la inscripción con estatus C=Inscrito académicamente y con R=Retiro de Asignaturas                      
                        if (daoAlumno.BuscarPeriodoInscriptoAcademico(daoConexion.ConexionBD(),"'C','R'")) {
                            ...
                            misession.setAttribute("incripcionAlumnoSession", 
                                "<tr><td><img src=\"/sicoes/recursos/img/error_1.gif\"/></td><td colspan=\"4\" align=\"center\"> Debe estar inscrito(a) académicamente, <br> posiblemente está realizando esta operación fuera de la fecha establecida o <br> ya realizó la adición de la(s) asignatura(s) correspondiente.</td></tr>");
                        }
                        ...
                        try {                            
                            if(materias!=null && materias.length>0) {                                
                                ...
                                for(int i=0; i<materias.length; i++) {                
                                    ...
                                    if (valoresSecciones!=null && valoresSecciones.length>2) {
                                        for(int k=2; k < valoresSecciones.length; k++) {
                                            // Para encadenar en la variable de horrarios las asignaturas de Pantalla
                                            if (horariosMaterias=="") {
                                ...
                                //Adicionar los Horarios a la variable de las asignaturas Inscritas que están en Base de Datos                                            // Para encadenar en la variable de horrarios las asignaturas de Pantalla
                                if (horariosMaterias=="") {
                                    horariosMaterias = daoAlumno.BuscarHorariosMateriasInscritas(daoConexion.ConexionBD());;
                                    //seccionesSeleccionadas = valoresSecciones[1].substring(0,valoresSecciones[1].indexOf("/"));
                                } else {
                                    horariosMaterias = horariosMaterias + "*" + daoAlumno.BuscarHorariosMateriasInscritas(daoConexion.ConexionBD());;
                                    //seccionesSeleccionadas = seccionesSeleccionadas + ", "+valoresSecciones[1].substring(0,valoresSecciones[1].indexOf("/"));                                            
                                }
                                ...
                                String cantidaUC = daoAlumno.VerificarUCInscritas(daoConexion.ConexionBD(), materiasSeleccionadas, daoAlumno.getCodEspecialidad()) + "+" + daoAlumno.VerificarUCMateriasInscritas(daoConexion.ConexionBD());
                                ...
                                if (mensaje=="") {                                
                                    // Guardar la Planilla.
                                    daoAlumno.GuardarAdicionInscripcionAlumno(daoConexion.ConexionBD(), materiasSeleccionadas, seccionesSeleccionadas);                                    
                                    ...
                                    String subject = "Adición de Asignatura(s) "+daoAlumno.getPeriodo();
                                    ...
                                    ":</p>" +
                                    "<p>Usted ha realizado la adición de Asignatura(s) a la Inscripción correspondiente al período académico <u>"+daoAlumno.getPeriodo()+"</u>, el cual ha sido procesada satisfactoriamente. </p>" +
                            ...
                            pagina="principal/adicionInscripcion";                   
                    ...
                    if (daoAlumno.BuscarPeriodoInscriptoAcademico(daoConexion.ConexionBD(),"'C','R'")) {
                        ...
                        //Buscar materias a adicionar                       
                        misession.setAttribute("incripcionAlumnoSession", daoAlumno.BuscarMateriasPorAdicionar(daoConexion.ConexionBD()));
